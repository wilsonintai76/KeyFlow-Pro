...
export default function Home() {
  ...
  useEffect(() => {
    async function checkAndInitialize() {
      ...
      // Ensure Wilson Poliku staff profile exists
      if (isMasterAdmin) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center space-y-8 bg-slate-50">
        ...
        <div className="w-full max-w-xs space-y-3">
          <Button onClick={() => auth && initiateGoogleSignIn(auth)} className="w-full h-12 gap-3 text-base shadow-md" variant="default">
